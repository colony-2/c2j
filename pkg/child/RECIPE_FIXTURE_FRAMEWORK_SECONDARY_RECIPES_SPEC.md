...
## Current Limitation
The fixture harness (in `/src/pkg/worker/test-fixtures`) registers only the primary recipe. The artifact path uses a registry that rejects unknown recipe IDs, so secondary recipe invocations fail.
...
## Documentation
Update `/src/pkg/worker/test-fixtures/HOW_TO_ADD_RECIPE_FIXTURE_TESTS.md` with:
