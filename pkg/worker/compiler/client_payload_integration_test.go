...
func TestClientPayloadSurvivesYieldAndExternalTaskCompletion(t *testing.T) {
	for _, backend := range []string{"toy", "sqlite"} {
		t.Run(backend, func(t *testing.T) {
			...
			require.Equal(t, jobworkflow.JobRunSuspended, run().Status)
...
func (payloadLifecycleWorker) Name() string { return "payload_lifecycle" }
func (w payloadLifecycleWorker) Run(ctx jobworkflow.JobContext, _ jobdb.JobData) (jobdb.JobData, error) {
	...
	return wrapped.DoTask(jobdb.DefaultRunPolicy(), "external", jobdb.NewTaskDataOrPanic(map[string]any{"prompt": true}))
}
