...
Explicit inputs must be sufficient. Listing a fully specified remote repository must not require Git, a checkout, current-directory discovery, or modification of process-wide environment variables. Fully explicit remote inputs must be unaffected by unrelated or invalid project configuration in the current directory or its ancestors. Different clients or calls in one process must be able to address different tenants and repositories without configuration leaking between them.
...
A caller must be able to fetch one logical page without automatically collecting all matching results. The page-size limit bounds returned results, not the amount of backend scanning. If execution filtering is exposed, filling a logical page may require examining multiple backend pages, potentially the entire remaining dataset when matches are sparse. Document this behavior; a separate scan limit is not requested. Document page limits, token lifetime or invalidation rules, and the requirement to keep filters consistent between pages. Preserve existing CLI/backend pagination semantics; a new transactional snapshot guarantee is not requested.
...
Execution requirements are exposed only for waiting jobs, according to existing c2j status semantics. For `ACTIVE` jobs, the execution view must indicate `in_flight` with requirements unavailable; previously recorded requirements must not be presented as current because in-flight requirements may be held ephemerally. Terminal jobs must report `not_waiting` with requirements unavailable.
...
2. With explicit connection, tenant, and repository inputs, it works without the c2j executable, Node.js/npm, a shell, Git, or a repository checkout. Tests verify that unrelated or invalid project configuration in the current directory or its ancestors does not affect fully explicit remote listing.
...
4. Results correctly distinguish specified, unspecified, unresolved, unavailable, and invalid execution information according to existing c2j semantics. For waiting jobs, effective requirements reflect changes already recorded by c2j. Tests also cover `ACTIVE` and terminal jobs with previously recorded requirements, verifying that those requirements are unavailable rather than presented as current.
5. Pagination, empty pages, invalid inputs, remote errors, and context cancellation have documented, tested behavior. If execution filtering is exposed, tests cover sparse matches across backend pages and verify the returned-result limit and continuation behavior.
