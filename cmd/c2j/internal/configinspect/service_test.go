...
`)
	...
	out := stdout.String()
	for _, want := range []string{"short_name", "cheetah", "repo", "github.com/acme/boo-cheetah", "root_repo", "github.com/acme/boo-root", "pattern"} {
...
`)
	...
	if info.Repo != "github.com/acme/self" || info.RootRepo != "github.com/acme/root" || info.RootRef != "release" {
...
func TestRunSelfAutoDetectsGoBaseWithoutConfig(t *testing.T) {
	...
	if info.Repo != "github.com/acme/boo-cheetah" || info.ShortName != "cheetah" || info.RootRepo != "github.com/acme/boo-root" {
